...
for country in random_countries:
    ...
    capital = country['capital'][0] if 'capital' in country else "no data"
    flag_code = country['flags']['png'] if 'flags' in country else "no data" # Flagpedia links to svg and png flags
    ...
    VALUES (%s, %s, %s, %s)
    """
    cursor.execute(insert_query, (name, capital, flag_code, population))
...
for row in rows:
    print(f"{row[0]}- {row[1]},\t\tcapital: {row[2]},\t\tflag code: {row[3]},\tpop: {row[4]}")
